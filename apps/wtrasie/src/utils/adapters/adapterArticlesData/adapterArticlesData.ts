...
export function adapterArticlesData(getArticles: GetArticlesQuery): Array<PostListData> {
  ...
  return getArticles.articles.data.map(art => ({
    ...
    cover: adapterImageData( { image: art.attributes?.cover.data?.attributes, typeImg: "medium" }),
