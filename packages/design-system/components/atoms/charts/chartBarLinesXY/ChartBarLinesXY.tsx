...
import type {ChartBarLineXYProps, ChartBarLineXYAccessor, ChartBarLineXYData} from './types';
...
export function ChartBarLinesXY({linesData}: ChartBarLineXYProps) {
		const wrapperRef = useRef<HTMLDivElement>(null);
		const {width, height} = useResizeDivObserver(wrapperRef);
		
		const accessors: { xAccessor: ChartBarLineXYAccessor; yAccessor: ChartBarLineXYAccessor } = {
				xAccessor: (d) => d.x,
				yAccessor: (d) => d.y,
		};
