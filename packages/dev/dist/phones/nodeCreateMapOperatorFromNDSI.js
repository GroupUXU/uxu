...
const phoneOperatorMap_1 = require("./phoneOperatorMap");
const phoneNumberRangeMap_1 = require("./phoneNumberRangeMap");
// Define the maps to store operator and range data
const phoneOperatorMap = new Map();
const phoneNumberRangeMap = new Map();
for (const [key, value] of phoneOperatorMap_1.phoneOperatorMapData) {
    phoneOperatorMap.set(key, value);
}
for (const [key, value] of phoneNumberRangeMap_1.phoneNumberRangeMapData) {
...
// Read the CSV file
fs_1.default.createReadStream('plik.csv')
    ...
    fs_1.default.writeFile('phoneOperatorMapData.ts', `export const operatorMap = new Map(${phoneOperatorMapJSON});`, (err) => {
        if (err)
            throw err;
        console.log('phoneOperatorMapData.ts has been saved!');
    });
    // Write rangeMap to a TypeScript file
    fs_1.default.writeFile('phoneNumberRangeMapData.ts', `export const rangeMap = new Map(${phoneNumberRangeMapJSON});`, (err) => {
        if (err)
            throw err;
